function p1fem1convect(datafile,givenfunc,Tmax)
% read mesh data
[deg,p,idp,t,idt,e,ide,d,idd] = getmesh(datafile);
% diffusion coef
coef_d=0.01;
% stiffnes/mass matrices
[K, M, C, kappa, bmax] = matrix1bt(p,t,givenfunc); 
Id = eye(size(K)); B = diag(1./diag(M))*(coef_d*K + C);
% initial value
usol=givenfunc(1,p(1,:),p(2,:))'; 
fign=1; figure(fign); plot_p1fem(p,t,usol); 
...
for n=1:nmax   
    ...
    %% for 2d & 3d draw
    if rem(n, step)==0
        fign=fign+1; figure(fign); plot_p1fem(p,t,usol); 
        ptime=[ptime;tnow];
...
xlabel('time');ylabel('mass');saveas(fign,"p1fem1convect_m.pdf");
% important values
kappa
tau
%%% end of program
