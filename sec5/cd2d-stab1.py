# diffusion-convection equation with SUPG stabilization
# 2021-10-18
...
#mesh = UnitSquareMesh(40, 40, diagonal="right")
mesh = Mesh("square1.xml")
#mesh = Mesh("square2.xml")
#mesh = Mesh("square3.xml")
#V = FunctionSpace(mesh, "CG", 1) #for P1 element
V = FunctionSpace(mesh, "CG", 2)  #for P2 element
...
a = nu*inner(grad(u), grad(v))*dx + inner(b, grad(u))*v*dx + tau*inner(b, grad(u))*inner(b, grad(v))*dx
L = f*v*dx + tau*f*inner(b, grad(v))*dx
u = Function(V)
solve(a == L, u, bc)
