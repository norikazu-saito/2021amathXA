function [R,rho] = boundary1(p,e,coef_kappa,givenfunc)
...
for k = 1:ne
  ...
  % for mass matrix 
  Rlocal=[2 1; 1 2]*length/6; R(elocal,elocal)=R(elocal,elocal)+Rlocal;
  % for right-hand side vector
  % xc = mean(x); yc = mean(y); rlocal = tmp*[1; 1]*length/2;
